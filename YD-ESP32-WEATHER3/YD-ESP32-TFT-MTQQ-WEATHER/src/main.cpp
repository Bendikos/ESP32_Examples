...
/*重要：
1.天气接口用的是易客天气，你们需要自己在https://yiketianqi.com/ 花一分钟时间注册一个账号，然后将common.h文件里的nowURL和futureURL后面的XXXX换成你们自己的appid和appsecret。
2.LCD彩屏的A引脚需要接电阻，不然有可能会烧坏电路，或者发热严重。
*/
...
void initDatas()
{
    ...
    while (!queryNowWeatherSuccess)
    {
        ...
        if ((end - start) > synDataRestartTime)
        {
            restartSystem("查询天气失败", true);
        }
        getNowWeather();
    }

    Serial.println("查询一周天气...");
...
void setup()
{
    ...
    pinMode(GPIO_NUM_22, OUTPUT);   // TFT的BL脚，一直拉高
    ...
    // TODO: 测试的时候，先写入WiFi信息，省的配网，生产环境请注释掉
    //setInfo4Test();
