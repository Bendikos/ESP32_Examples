...
#define GPIO_OUTPUT_PIN_SEL ((1ULL << LCD_PIN_RES) | (1ULL << LCD_PIN_DC) | (1ULL << LCD_PIN_BLK))

spi_device_handle_t spi_handle1;
...
void lcdGpioInit(void)
{
	...
	esp_err_t ret = spi_bus_initialize(SPI2_HOST, &bus_cfg, 0);
	if (ret != ESP_OK)
	{
	...
	spi_device_interface_config_t dev_cfg1 = {
		.clock_speed_hz = 30000000, // 30 MHz
		.mode = 0,					// SPI模式0
		.spics_io_num = PIN_NUM_CS_1,
		.queue_size = 7,
	};
	ret = spi_bus_add_device(SPI2_HOST, &dev_cfg1, &spi_handle1);
	if (ret != ESP_OK)
	{
		ESP_LOGE(TAG, "Failed to add SPI device 1");
