...
PN532_RES PN532_Write_WaitAck(uint8_t *pbuf, uint16_t len, uint32_t timeout)
{
    ...
    ESP_ERROR_CHECK(i2c_master_receive(pn532_handle, res, 7, 100));
...
PN532_RES PN532_Response(uint8_t *pbuf, uint16_t len, int timeout)
{
    ...
    ESP_ERROR_CHECK(i2c_master_receive(pn532_handle, pbuf, len, 1000));
    return PN532_OK;
}
...
PN532_RES PN532_Get_Version(uint8_t *ver)
{
    ...
    if (PN532_Write_WaitAck(pack, ret, 200) != 0)
    {
        return PN532_TIMEOUT;
    }

    if (PN532_Response(res, 16, 100) != 0)
    {
        return PN532_TIMEOUT;
    }
